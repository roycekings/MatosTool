...
const EncoursUsers = () => {
  ...
  const getData = async ()=>{
    ...
    try {
      const response = await axios.get(`http://localhost:3001/api/user/getencouruser/${user._id}`,{
        headers: {
          authorization: token
        }
      })
  ...
  return (
    <div>
      <h1 className=' text-center text-3xl text-blue-500 py-3 font-bold'>
        Liste d'emprunt en cour
      </h1>
      {
        data.map((d,index)=>(
          
          <div className=' mb-10 py-1 my-1 border shadow px-2 rounded'>
