...
const FormMatosUpdate = () => {
  ...
  const getMatos = async ()=>{
    try {
      const response = await axios.get(`http://localhost:3001/api/materiel/getOne/${id}`,{
...
console.log(data);
  ...
  const hanleSubmit = async (e)=>{
    ...
    try {
      const response = await axios.put(`http://localhost:3001/api/materiel/update/${id}`,data,{
        headers: { authorization: token }
      })
  ...
  return (
    ...
    >   
            <h1 className=' font-bold text-2xl text-blue-400 text-center py-4' > Modification de materiel </h1>
        ...
        <div className=' mb-4 flex'>
                <SubmitButton onClick={hanleSubmit} value='Modifier' />    
        </div> 
...
export default FormMatosUpdate