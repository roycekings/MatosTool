...
const UserProfile = () => {
    ...
    const handleDelete = async () =>{
        ...
        try {
            const deleteUser = await axios.put(`http://localhost:3001/api/profil/delete/${id}`,data,{
                headers:{
                    authorization: token
                }
            })
  ...
  return (
   <div>
    ...
    <div className='  grid grid-cols-5 border p-3 shadow-md my-3 h-auto '>
        <Link to='/user/demandeenattente' className='py-2 px-2 border rounded-md text-white bg-blue-600 shadow-md flex gap-2 items-center'><p className='mx-auto'>Demande  d'emprunt en attente</p></Link>
        <Link to='/user/encours' className='py-2 px-2 border rounded-md text-white bg-green-600 shadow-md flex gap-2 items-center'><p className='mx-auto'>Emprunt en cours</p></Link>
        <Link to='/user/historique' className='py-2 px-2 border rounded-md text-white bg-gray-600 shadow-md flex  gap-2 justify-center items-center'><p>Historique  d'emprunt</p><FaHistory className='w-4 h-4' /></Link>
        ...
        <Link onClick={handleDelete} className='py-2 px-2 border rounded-md text-white bg-red-600 shadow-md flex gap-2 items-center justify-center'><p>Supprimer Compte</p><MdDeleteForever /></Link>
