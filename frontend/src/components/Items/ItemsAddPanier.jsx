...
const ItemsAddPanier = ({qte, nom,marque,modele,poids,categorie,typeConnectique,handleAdd}) => {
    ...
    <div className='w-full border h-auto justify-between items-center flex bg-transparent  py-3 px-9 shadow-md' >
      <div className=' w-4/5  grid grid-cols-6'>
        ...
        <div className=' flex flex-col gap-2'>
          <p className='border px-3 py-2 bg-slate-700 text-sky-50 rounded shadow-md font-semibold'><span className='font-normal'>quantite disponible:</span> {qte}</p>
          <button onClick={handleAdd} className='border px-3 py-2 bg-blue-700 text-sky-50 rounded shadow-md font-semibold' >Ajouter au panier</button>
        </div>
    </div>
  )
}

export default ItemsAddPanier