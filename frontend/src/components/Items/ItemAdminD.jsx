...
const ItemAdminD = ({_id,status,hour,nomUser,prenomUser, nom,marque,modele,poids,categorie,typeConnectique,handleAccept,handleRefuse}) => {
  return (
    <div>
  ...
  <div className=' float-right mr-4 py-1'>
    <button onClick={handleAccept} className=' border px-3 py-1 bg-green-500  text-white rounded  '>accepter</button>
    <button onClick={handleRefuse} className=' border px-3 py-1 bg-red-500  text-white rounded  '>refuser</button> 
  </div>
    </div>
  )
}

export default ItemAdminD